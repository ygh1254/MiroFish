...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### 분석한 하위 질문\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    const queryMatch = text.match(/질의:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    const nodesMatch = text.match(/전체 노드 수:\s*(\d+)/)
    const edgesMatch = text.match(/전체 엣지 수:\s*(\d+)/)
    ...
    const activeSection = text.match(/### 현재 유효한 사실[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^|^|^선택/)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        const twitterMatch = answerText.match(/Twitteranswer\n?([\s\S]*?)(?=Redditanswer|$)/)
        const redditMatch = answerText.match(/Redditanswer\n?([\s\S]*?)$/)
    ...
    const summaryMatch = text.match(/### 인터뷰\n([\s\S]*?)$/)
    if (summaryMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, ` (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, ` (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Querygenerationgrouped'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'None'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'None'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'None')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `has (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Nonehas'),
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Not found'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? ` ▲` : `all ${props.result.facts.length}  ▼`)
