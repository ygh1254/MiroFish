    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">generationcompleteafterStart graph build</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">buildProcess</span>
        </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  UploadafterLLMgroupedcontentgenerationtypes + types
                </div>
              </div>
              
              <!-- Ontology generationprogress -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">generationprogress</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">generationtypes ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">generationtypes ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API</div>
                <div class="detail-content">
                  generationchunkingafterCall Zep API build
                </div>
              </div>
              
              <!-- Waitingcomplete -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">WaitingOntology generation completed...</div>
              </div>
...
const ontologyProgress = ref(null) // Ontology generationprogress
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0) return 'Ontology generation'
  return 'Initialize'
})
...
const goToNextStep = () => {
  // TODO: Go to environment setup
  alert('Environment setupfeature...')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'hasUploadreturnsHome'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Ontology generation
    ontologyProgress.value = { message: 'Uploadgrouped...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Ontology generation'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'Initialize: ' + (err.message || 'Unknown error')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Save task_id used topolling
      const taskId = response.data.task_id
      
      // datapollingstatepolling
      startGraphPolling()
...
const fetchGraphData = async () => {
  try {
    // Get project informationGet graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph build completedata...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // updateprogresscompletestate
        buildProgress.value = {
          progress: 100,
