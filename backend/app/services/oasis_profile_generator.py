...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "你是社交媒体用户画像生成专家。生成详细、真实的人设用于舆论模拟,最大程度还原已有现实情况。必须返回有效的JSON格式，所有字符串值不能包含未转义的换行符。默认使用韩语输出，但必须保留实体真实的文化背景和国别信息。"
        return base_prompt
...
6. country: 国家（使用韩语表记，如"한국"、"일본"、"미국"）
7. profession: 职业
...
- persona必须是一段连贯的文字描述
- 默认使用韩语（除了gender字段必须用英文male/female）
- 内容要与实体信息保持一致
