...
【输出要求】标题、摘要、章节标题和描述必须全部使用韩语。"""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
你是一个「未来预测报告」的撰写专家，正在撰写报告的一个章节。所有最终输出必须使用自然、专业的韩语。
...
3. 【语言一致性 - 引用内容必须翻译为报告语言】
   ...
   - 无论模拟需求和材料原文是什么语言，报告必须全部使用韩语撰写
   - 当你引用工具返回的英文或混合语言内容时，必须将其翻译为流畅的韩语后再写入报告
   - 翻译时保持原意不变，确保表述自然通顺
