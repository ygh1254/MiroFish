...
- 目标 locale: {locale_cfg["label"]}
- 活跃时段需要符合目标 locale 的一般作息与平台使用习惯
- 一般规律：凌晨低活跃、早间渐增、工作/日间中等、晚间高峰
...
{{
    ...
    "reasoning": "针对该事件和目标 locale 的时间配置说明（韩语）"
}}
...
- reasoning (string): 简要说明为什么这样配置"""
        ...
        system_prompt = f"你是社交媒体模拟专家。返回纯JSON格式，时间配置需符合目标 locale（{locale_cfg['label']}）的使用习惯。"
...
```
        ...
        ## 任务
        为每个实体生成活动配置，注意：
        - **时间符合目标 locale（{LOCALE_TIME_CONFIGS.get(locale_code, LOCALE_TIME_CONFIGS["GLOBAL"])["label"]}）的作息习惯**
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<活跃小时列表，考虑目标 locale 的作息>],
        ...
        system_prompt = f"你是社交媒体行为分析专家。返回纯JSON，配置需符合目标 locale（{LOCALE_TIME_CONFIGS.get(locale_code, LOCALE_TIME_CONFIGS['GLOBAL'])['label']}）的作息习惯。"
