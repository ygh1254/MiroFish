"""
API retry utilities
Retry logic for external API calls such as LLM requests
"""
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(
                        ...
                        f"{current_delay:.1f}s before retry..."
                    )
...
class RetryableAPIClient:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    logger.error(f"API call still failed after {self.max_retries} still failed after retries: {str(e)}")
                    raise
    ...
    def call_batch_with_retry(
        ...
        """
        Run batched calls and retry each failed item individually
        
        Args:
            items: List of items to process
            process_func: Processing function that accepts a single item
            exceptions: Exception types that should trigger a retry
            continue_on_failure: Whether to continue after a single item fails
            
        Returns:
            (List of successful results and failed items)
        """
        results = []
        failures = []
