...
The Zep node/edge list APIs paginate with UUID cursors,
and this module wraps automatic pagination with single-page retries.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page with exponential backoff for transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes via pagination, up to max_items, with per-page retries."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges via pagination with per-page retries."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
