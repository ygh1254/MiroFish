...
Uses project context so state is persisted on the server
"""
...
def reset_project(project_id: str):
    """
    Reset project state (for graph rebuild)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    API 1: Uploadgroupedontology definition
    
    Methodmultipart/form-data
    ...
    try:
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Provide simulation_requirement (simulation_requirement)"
            }), 400
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "No document was processed successfully. Check the file format."
            }), 400
...
def build_graph():
    """
    API 2: Based onproject_idbuild graph
    ...
    try:
        ...
        logger.debug(f"Parameters: project_id={project_id}")
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "has not generated an ontology yetCall /ontology/generate"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "The graph is already building. To force a rebuild, send force: true",
                "task_id": project.graph_build_task_id
            }), 400
        
        # If force rebuild is enabled, reset the state
        ...
        if not ontology:
            return jsonify({
                "success": False,
                "error": "Not founddefinition"
            }), 400
        
        # Create async task
        task_manager = TaskManager()
        task_id = task_manager.create_task(f"build graph: {graph_name}")
        logger.info(f"Create graph build task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            ...
            try:
                ...
                # addprogress_callback  (msg, progress_ratio)
                ...
                # WaitingZepcompleteQueryeachepisodeprocessedstate
                task_manager.update_task(
                    task_id,
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph build task started /task/{task_id} check progress"
            }
        })
